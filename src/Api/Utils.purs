...
-- | monad, `Puregit.AppM`.
module Puregit.Api.Utils where

import Prelude

import Affjax (request)
import Puregit.Api.Request (BaseURL, RequestOptions, Token, defaultRequest, readToken, writeToken)
import Puregit.Capability.LogMessages (class LogMessages, logError)
...
-- | `baseUrl` field from the app environment. See `Puregit.AppM` for examples of this in action.
mkRequest 
  :: forall m r
   . MonadAff m
...
-- | the asynchronous request. See `Puregit.AppM` for examples of this in action.
mkAuthRequest 
  :: forall m r
   . MonadAff m
  => MonadAsk { baseUrl :: BaseURL | r } m
...
-- | dealing with malformed responses. See `Puregit.AppM` for examples of this in practice.
