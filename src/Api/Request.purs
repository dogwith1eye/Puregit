...
-- | functions that our production monad will leverage in `Puregit.AppM` to implement our various
-- | app capabilities.
module Puregit.Api.Request 
...
import Puregit.Api.Endpoint (Endpoint(..), endpointCodec)
import Puregit.Data.Email (Email)
import Puregit.Data.Profile (Profile, decodeProfile)
...
-- |
-- | Check out the `Puregit.Capability.Resource.*` modules for examples of the high-level functions
...
-- | request functions defined below and in the `Puregit.Api.Utils` modules.
...
-- |
-- | Both functions -- as well as functions from the `Puregit.Capability.Resource.User` module -- 
...
-- | However, in the `Puregit.Capability.Resource.User` module, we'll see another record using  
...
-- | particular key. They'll be used as part of our production monad, `Puregit.AppM`.
