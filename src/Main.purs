...
import Puregit.Api.Request (BaseURL(..), RequestMethod(..), defaultRequest, readToken)
import Puregit.Api.Utils (decodeAt)
import Puregit.AppM (Env, LogLevel(..), runAppM)
import Puregit.Component.Router as Router
import Puregit.Data.Route (Route, routeCodec)
...
main = HA.runHalogenAff do
  ...
  -- Our environment is a small record type, `Env`, defined in the `Puregit.AppM` module. It 
  ...
  let 
    baseUrl = BaseURL "https://api.github.com/graphql"
    logLevel = Dev
  ...
  --
  -- The `runAppM` function we wrote in the `Puregit.AppM` module provides this transformation from 
  ...
  -- `Query` and `Message` types defined in the `Puregit.Router` module. Only those queries and 
